...
void combGammaPlots()
{
  ...
  for (int ic=1;ic<9;ic++)
  {
    ...
    fcombsp[ic]->Draw();
    fcombsp[ic]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
    fcombsp[ic]->GetYaxis()->SetTitle("dN/dp_{T} (GeV^/c)^{-1}");
    gpi0_gamma_sp[ic]->SetLineColor(kBlue);
    geta_gamma_sp[ic]->SetLineColor(kMagenta);
    ...
    gPad->SetLogy(1);
    TLegend* legsp = new TLegend(0.45,0.6,0.88,0.9);
    ...
    leg->Draw("same");
    /* drawLatex( 0.24, 0.19, Form("%s%s",centnamestring[ic].Data() ,"%"),0.05); */
