...
void compare2PE_my()
{
  ...
  // drawDecayLplot("pair DecayLength",pdf,c,realdata,"rescaleFile/rescale_gamma_dirpho.root","1");
  //
...
void drawDecayLplot(TString head, TPDF* pdf, TCanvas* c,TString real,TString mc,TString scale)
{
   ...
   // int const nbins = 7;
   // double ptedge[nbins+1]={0.25,0.3,0.4,0.6,1.0,1.4,1.8,3};
   TFile* fdecatLforPlots = new TFile(Form("output/fdecayLforPlots_%s.root",scale.Data()),"recreate");
   hdata->Write();
   hrc->Write();
   fdecatLforPlots->Close();
}
