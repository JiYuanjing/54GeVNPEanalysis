...
void fitEffFun(double* x, double* p)
{
   ...
   else if (x[0]>1.1&&x[0]<1.1) return fun1(x[0],p[4],p[5],p[6],p[7]);
   // else if (x[0]>1.5) return fun1(x[0],p[8],p[9],p[10],p[11])-fun1(1.5,p[8],p[9],p[10],p[11])+fun1(1.5,p[4],p[5],p[6],p[7]);
   else if (x[0]>1.1) return fun1(x[0],p[8],p[9],p[10],p[11]);
}
...
void fitEff()
{
  ...
  for (int i=0;i<Npx;i++)
  {
    ...
    double stat = hRecoEff->GetBinError(hRecoEff->GetXaxis()->FindBin(x[i]));
    if (x[i]<0.3) err[i]=y[i]*gtoterr->Eval(0.3);
    else if (x[i]>2.6) err[i]=y[i]*gtoterr->Eval(2.6);
    else err[i]=y[i]*gtoterr->Eval(x[i]);
    /* if (x[i]<0.5) cout << stat/err[i]<<" "<<stat/y[i] <<endl; */
    /* err[i] = sqrt(err[i]*err[i]+stat*stat); */
    /* err[i] = sqrt(stat*stat); */
  }
