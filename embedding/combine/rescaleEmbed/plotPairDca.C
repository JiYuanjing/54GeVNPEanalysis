...
{
  SetMyStyle();
  TFile* f100 = TFile::Open("output/fPairDca_1.root");
  TFile* f85 =  TFile::Open("output/fPairDca_0.7.root");
  TFile* f115 =  TFile::Open("output/fPairDca_1.3.root");
  ...
  gasymm->SetName("gPairDca");
  hdata->SetMarkerSize(1);
  hdata->GetYaxis()->SetTitle("Arb. unit");
  hdata->GetXaxis()->SetTitle("Pair DCA [cm]");
  hdata->Draw("p");
  ...
  gPad->SaveAs("pdf/PairDca.pdf");
  gPad->SaveAs("pdf/PairDca.png");

  TFile* fout = new TFile("output/gPairDca.root","recreate");
  hMC100->Write();
  hMC115->Write();
  hMC85->Write();
  hdata->Write();
