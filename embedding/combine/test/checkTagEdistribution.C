...
void checkTagEdistribution()
{
  ...
  /* TFile* file = new TFile("embeddQa_gamma_Apr28_3.root"); */
  TFile* file = new TFile("embedd0824.root");
  ...
  TH1F* h1TagEcut = (TH1F*)hTagEcut->ProjectionX("h1TagEcut",3,9);
  h1TagEcut->Rebin();
  ...
  h1TagEcut->GetYaxis()->SetRangeUser(0,0.65);
  h1TagEcut->GetYaxis()->SetTitle("Eff.");
  h1TagEcut->GetXaxis()->SetRangeUser(0,4);
  h1TagEcut->Draw();
  ...
  leff->AddEntry(hrestcut,"Start radius>40 cm","p");
  leff->AddEntry(htestMc[8],"Start radius<40 cm","p");
  ...
  lsp->AddEntry(hrest,"Start radius>40 cm","p");
  lsp->AddEntry(htestMc[7],"Start radius<40 cm","p");
  ...
  TH3F* hCheckVtx[2];
  hCheckVtx[0] = (TH3F*)file->Get("hCheckVtx_3"); 
  ...
  for (int i=0;i<2;i++) 
  {
    ...
    if (i==0) hCheckVtx_xy[i]->GetZaxis()->SetRangeUser(1e-7,1);
    if (i==1) hCheckVtx_xy[i]->GetZaxis()->SetRangeUser(1e-6,10);
  ...
  TH2F* hDecayL3 = (TH2F*)file->Get("hPairDecayL3"); 
  TH2F* hDecayL2 = (TH2F*)file->Get("hPairDecayL2"); 

  c1->cd(1);
  hDecayL3->Draw("colz");
  drawLatex(0.2,0.8,"Mom res>0.06");
  c1->cd(2);
  hDecayL2->Draw("colz");
  drawLatex(0.2,0.8,"Mom res<0.06");
  ...
  c1->cd(1);
  TH2F* hParent2 = (TH2F*)file->Get("hElectronVsParent2");
  TH2F* hParent3 = (TH2F*)file->Get("hElectronVsParent3");
  TProfile* pparent2 = (TProfile*)hParent2->ProfileX();
  TProfile* pparent3 = (TProfile*)hParent3->ProfileX();

  pparent3->SetLineColor(kBlue);
  pparent2->SetLineColor(kRed);
  pparent3->SetMarkerColor(kBlue);
  pparent2->SetMarkerColor(kRed);

  pparent2->GetYaxis()->SetRangeUser(0,5);
  pparent2->Draw();
  pparent3->Draw("same");
  TLegend* legpparent = new TLegend( 0.2,0.7,0.6,0.88 );
  legpparent->AddEntry( pparent3, "Start radius>40 cm","pe" );
  legpparent->AddEntry( pparent2, "Start radius<40 cm","pe" );
  legpparent->Draw();

  c1->cd(2);
  TH2F* hDcaPair3= (TH2F*)file->Get("hDcaPair3");
  TH2F* hDcaPair2= (TH2F*)file->Get("hDcaPair2");
  TProfile* pDcaPair2 = (TProfile*)hDcaPair2->ProfileY();
  TProfile* pDcaPair3 = (TProfile*)hDcaPair3->ProfileY();

  pDcaPair3->SetLineColor(kBlue);
  pDcaPair2->SetLineColor(kRed);
  pDcaPair3->SetMarkerColor(kBlue);
  pDcaPair2->SetMarkerColor(kRed);

  pDcaPair3->GetYaxis()->SetRangeUser(0,1);
  pDcaPair3->Draw();
  TLegend* leg = new TLegend( 0.2,0.7,0.6,0.88 );
  leg->AddEntry( pDcaPair3, "Start radius>40 cm","pe" );
  leg->AddEntry( pDcaPair2, "Start radius<40 cm","pe" );
  leg->Draw();
  pDcaPair2->Draw("same");

  addpdf(pdf,c1);
  c1->cd(1);
  TH2F* hPartEnFitsTagEpt3= (TH2F*)file->Get("hPartEnFitsTagEpt3");
  TH2F* hPartEnFitsTagEpt2= (TH2F*)file->Get("hPartEnFitsTagEpt2");
  // TProfile* pPartEnFitsTagEpt2 = (TProfile*)hPartEnFitsTagEpt2->ProfileX();
  // TProfile* pPartEnFitsTagEpt3 = (TProfile*)hPartEnFitsTagEpt3->ProfileX();
  //
  // pPartEnFitsTagEpt3->SetLineColor(kBlue);
  // pPartEnFitsTagEpt2->SetLineColor(kRed);
  // pPartEnFitsTagEpt3->SetMarkerColor(kBlue);
  // pPartEnFitsTagEpt2->SetMarkerColor(kRed);
  //
  // pPartEnFitsTagEpt3->GetYaxis()->SetRangeUser(20,50);
  // pPartEnFitsTagEpt3->Draw();
  // TLegend* leg = new TLegend( 0.2,0.7,0.6,0.88 );
  // leg->AddEntry( pPartEnFitsTagEpt3, "Mom res>0.06","pe" );
  // leg->AddEntry( pPartEnFitsTagEpt2, "Mom res<0.06","pe" );
  // leg->Draw();
  // pPartEnFitsTagEpt2->Draw("same");
  hPartEnFitsTagEpt3->Draw("colz");
  drawLatex(0.2,0.8,"Start radius>40 cm");
  gPad->SetLogz();
  c1->cd(2);
  hPartEnFitsTagEpt2->Draw("colz");
  drawLatex(0.2,0.8,"Start radius>40 cm");
  gPad->SetLogz();
  addpdf(pdf ,c1);

  TH2F* hPartEpt3= (TH2F*)file->Get("hPartEpt3");
  TH2F* hPartEpt2= (TH2F*)file->Get("hPartEpt2");
  c1->cd(1);
  // gPad->SetLogz();
  // hPartEpt3->Draw("colz");
  // drawLatex(0.2,0.8,"Mom res>0.06");
  ...
  // hPartEpt2->Draw("colz");
  // drawLatex(0.2,0.8,"Mom res<0.06");
  TProfile* pPartEpt2 = (TProfile*)hPartEpt2->ProfileY();
  TProfile* pPartEpt3 = (TProfile*)hPartEpt3->ProfileY();

  // if (int i=0;i<;i++)
  // {
  pPartEpt3->SetLineColor(kBlue);
  pPartEpt2->SetLineColor(kRed);
  pPartEpt3->SetMarkerColor(kBlue);
  pPartEpt2->SetMarkerColor(kRed);

  pPartEpt3->GetYaxis()->SetRangeUser(0,1.);
  pPartEpt3->GetXaxis()->SetTitle("Tagged e p_{T}");
  pPartEpt3->Draw();
  pPartEpt2->Draw("same");
  // }
  // TLegend* leg = new TLegend( 0.2,0.7,0.6,0.88 );
  // leg->AddEntry( pPartEpt3, "Mom res>0.06","pe" );
  // leg->AddEntry( pPartEpt2, "Mom res<0.06","pe" );
  // leg->Draw();
  //
  addpdf(pdf,c1);
  //
  // double pt[10]={0.5,0.6,0.7,0.8,0.9,1,1.2,1.5,2};
  // for (int ip=0;ip<;ip++)
  // {
  // }
  ...
  c1->Divide(2,1);
  c1->cd(1);
  TH1F* hpartTrackeff[2];
  TH1F* hpartTot[2];
  TH1F* hpartPass[2];
  TH2F* hpart[2];
  for (int i=0;i<2;i++)
  {
     if (i==0) hpart[i] = (TH2F*)file->Get("hPartEtrack2");
     if (i==1) hpart[i] = (TH2F*)file->Get("hPartEtrack3");
     hpartTot[i] = (TH1F*)hpart[i]->ProjectionX(Form("hparttot%d",i),1,1);
     hpartPass[i] = (TH1F*)hpart[i]->ProjectionX(Form("hpartpass%d",i),2,2);
     hpartPass[i]->Divide(hpartTot[i]);
     hpartPass[i]->Draw("same");
     if (i==0) hpartPass[i]->SetLineColor(kRed);
     if (i==1) hpartPass[i]->SetLineColor(kBlue);
  }
  TLegend* legtrack = new TLegend(0.6,0.2,0.85,0.38);
  /* legtrack->AddEntry(hpartPass[0],"Mom Res<0.06","l" ); */
  /* legtrack->AddEntry(hpartPass[1],"Mom Res>0.06","l" ); */
  legtrack->AddEntry(hpartPass[0],"Start radius<40 cm","l" );
  legtrack->AddEntry(hpartPass[1],"Start radius>40 cm","l" );
  ...
  c1->Divide(2,1);
  c1->cd(1);
  TH2F* hMomResVsPos = (TH2F*)file->Get("hMomResVsPos");
  hMomResVsPos->GetYaxis()->SetRangeUser(0,70);
  hMomResVsPos->GetYaxis()->SetTitle("Start vertex radius in XY (cm)");
  hMomResVsPos->GetZaxis()->SetRangeUser(1e-8,10);
  hMomResVsPos->Draw("colz");
  gPad->SetLogz();
