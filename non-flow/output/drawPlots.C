...
float RefFlowMultAuAu = 122;
float RefFlowMultErrAuAu = 1e-02;

float RefFlowAuAu = 0.046;
float RefFlowErrAuAu = 1e-04;

// pp
const int nEtaGaps=1;
const int nPtBins=6;
float etagap[nEtaGaps]={0.05};
/* float ptBin[nPtBins+1]={0.2,0.4,0.6,0.8,1,1.2,1.4,1.6,1.8,2,2.5,3,3.5,4}; */
float ptBin[nPtBins+1]={0.2,0.4,0.6,0.8,1,1.2,4};
...
/* void drawPlots(const bool isFullRange=true) { */
  ...
  {
     inDir="fullrange";
     /* TFile *fin_s200GeV  = new TFile("fullrange_nowt.root"); */
     TFile *fin_s200GeV  = new TFile("fullrange_eta1.root");
  }
  else
  {
     inDir="halfrange";
     TFile *fin_s200GeV  = new TFile("halfrange_eta1.root");
  }

  TFile *fin_s200GeV_fullRange  = new TFile("fullrange_nowt.root");
  ...
  hHadron_Pt_PP[0]= (TH1F*)fin_s200GeV->Get(Form("hHad_noEtaGap_pTBin%d",nPtBins));
  hHadron_Pt_PP[1]= (TH1F*)fin_s200GeV->Get(Form("hHad_EtaGap_dot0_pTBin%d",nPtBins));
  cPtHadron->cd(1);
  ...
  /* for(auto i=0; i<4; i++){ */
    ...
    hDPhi_allTracks_PtBin[i]->GetYaxis()->SetRangeUser(0., 0.03);
    ...
    hDPhi_deltaEta01_PtBin[i]->GetYaxis()->SetRangeUser(0.008, 0.02);
    ...
    leg_dPhi[i]->SetHeader(Form("%0.2f <p_{T}< %0.2f GeV, |#eta|<1.", ptBin[i], ptBin[i+1]));
    ...
    leg_dPhi[i]->Draw();
    if (i%4==3 ) cDPhi->Print(Form("dPhi_%s_%d.pdf", inDir.Data(),i));
  }
  ...
  float downLimit_V2 = -0.02, upLimit_V2 = 0.1;
  if(!isFullRange) {
    downLimit_M2 = 5e-4;
    upLimit_M2 = 1.1;
    downLimit_V2 = -0.02;
  ...
  hM2[0]->GetXaxis()->SetRangeUser(0., 4);
  //hM2[0]->Draw("E1");
  ...
  hM2[1]->GetXaxis()->SetRangeUser(0, 4);
  ...
  leg_M2->SetHeader("0.2 < p^{h}_{T} < 2.0 GeV/c, |#eta_{h}|<0.8");
  ...
  hV2[0]->GetXaxis()->SetRangeUser(0, 4);
  //hV2[0]->Draw("E1");
  ...
  hV2[0]->SaveAs(Form("nonflow_nogap_%s.root",inDir.Data()));
  ...
  hV2[1]->GetXaxis()->SetRangeUser(0, 4);
  ...
  hV2[1]->SaveAs(Form("nonflow_%s.root",inDir.Data()));
  ...
  leg_V2->SetHeader("0.2 < p^{h}_{T} < 2.0 GeV/c, |#eta_{h}|<1");
